...
#Getting help with functions and features (similar to 'man' in unix):
help(mean)          #This will provide you help for the mean function
?min                #This is an alternative method to find help (same thing diff syntax). This will give you help with the "min" function
help("[[")          #This will provide help with syntax.
help.start()        #This will launch the help manual.
?help.search        #Allows for searching the help manual system
help.search("linear models")  #Searches for help about linear models

#Getting examples on a help topic:
example(matrix)  #provides an example of a matrix

#Syntax 
x <- 1; y <- 2      #commands are seperated either by ";" or by a new line
x+y
x <- c(1,2,3,4,5,6)      #creating a vector
mean(x)                  #mean
median(x)                #median
getwd()                  #get working directory
setwd(EnterPathToNewDir) #set a working directory, in Windows slashes need to be '//'
a <- c(4,7,5,6,12,0,1)
sort(a)                  #sorts the vector "a"
cumsum(x)                #Adds the first number then the first two numbers, then the first three, etc...

y<- c(2,6,7,9,7,NA)
mean(y)               #will output: NA
mean(y, na.rm = TRUE) #works, it removes NA from dataset 

rm(x)  #remove objects from R history, objects are stored in the working directory
?lm() #linear model 

#MATHWORLD: is a good resource for this class!
#Use R!: is a great book

#Comparing generating exponential random variables
#set.seed(123)  
n = 1000      #generating ten random samples (as this number is increased: a,b converge)
lamda = 3   #
a <- rexp(n,lamda)
b <- -(1/lamda)*log(1-runif(n,0,1))
mean(a)
mean(b)
hist(a)
hist(b)

#Find out the relationship between pnorm and qnorm:
pnorm(-1.9645)
qnorm(0.25) #area to the left of the curve

#Rounding
ceiling(a) #round up
floor(b)   #round down
help("trunc")
round(a,digits=10)  #round to nth digits

#Working with a default R dataset called iris
data(iris3)
class(iris3)
dim(iris3)
iris3
...
##Non-parametric methods are robost but not only
#Training Data, we are just trying to get information from what we have
#Linear Regression: easiest is least squares
...
passedvalues
plot(x=myvalues, y=passedvalues, main = "Plot your function from part (a), x ranges from [-2, 2]",
     xlab = "x", ylab = "f(x)",
